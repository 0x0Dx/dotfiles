...
start_hypr_mon_guard(){
  if ! pgrep -x hypr_mon_guard >/dev/null 2>&1; then
    if PATH=$PATH:${REPO_ROOT}/packages/install/subcmds/virtmon command -v hypr_mon_guard ; then
      echo "Running hypr_mon_guard."
      PATH=$PATH:${REPO_ROOT}/packages/install/subcmds/virtmon setsid hypr_mon_guard > $(mktemp) 2>&1 &
    else
