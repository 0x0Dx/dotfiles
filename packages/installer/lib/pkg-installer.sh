...
install-python-packages(){
  UV_NO_MODIFY_PATH=1
  DAIFUKU_VIRTUAL_ENV=$XDG_STATE_HOME/quickshell/.venv
  x mkdir -p $(eval echo $DAIFUKU_VIRTUAL_ENV)
  # we need python 3.12 https://github.com/python-pillow/Pillow/issues/8089
  x uv venv --prompt .venv $(eval echo $DAIFUKU_VIRTUAL_ENV) -p 3.12 --clear
  x source $(eval echo $DAIFUKU_VIRTUAL_ENV)/bin/activate
  if [[ "$INSTALL_VIA_NIX" = true ]]; then
    x nix-shell ${REPO_ROOT}/packages/installer/uv/shell.nix --run "uv pip install -r ${REPO_ROOT}/packages/installer/uv/requirements.txt"
  else
    x uv pip install -r ${REPO_ROOT}/packages/installer/uv/requirements.txt
  fi
  x deactivate
}
