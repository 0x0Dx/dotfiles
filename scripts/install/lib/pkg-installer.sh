...
install-python-packages(){
  ...
  if [[ "$INSTALL_VIA_NIX" = true ]]; then
    x nix-shell ${REPO_ROOT}/scripts/install/uv/shell.nix --run "uv pip install -r ${REPO_ROOT}/scripts/install/uv/requirements.txt"
  else
    x uv pip install -r ${REPO_ROOT}/scripts/install/uv/requirements.txt
  fi
  x deactivate
}
